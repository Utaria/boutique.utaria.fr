<div class="wrap-inner col-group content">
	...
	<form method="POST" action="" class="col-6 col-offset-3 login-form">
		<div class="input col-group<?= ($error) ? ' error' : '' ?>">
			<label for="playername" class="col-4">Pseudonyme</label>
			<input type="text" class="col-8" name="playername" id="playername" placeholder="Pseudo sur le serveur" required>
		</div>
		<div class="input col-group<?= ($error) ? ' error' : '' ?>">
			...
			<?php if ($error): ?>
				<p class="error col-offset-4 col-8"><?= $error ?></p>
			<?php endif; ?>
		</div>
